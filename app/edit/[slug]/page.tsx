...
import type { FormSettings, FormQuestion, QuestionType, FormDetail } from "@/types/form"
...
type PageState = "loading" | "editing" | "submitting" | "success"
...
export default function EditFormPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const form: FormDetail = await fetchFormBySlug(slug)
        if (!mounted) return
        setTitle(form.title)
        setDescription(form.description || "")
        setSettings(form.settings || { status: "draft", restricted: false })
        setQuestions(
        ...
        // Note: allowed users list isn't returned by fetchFormBySlug in current client API,
        // so we leave restrictedEmails empty for manual editing.
        setState("editing")
      } catch (err) {
        setState("editing")
        setErrors({ submit: err instanceof ApiRequestError ? err.message : "Failed to load form" })
      }
    })()
    return () => {
      mounted = false
  ...
  const handleSubmit = async () => {
    ...
    setState("submitting")
    try {
      const updated = await updateForm(slug, {
        title: title.trim(),
      ...
      setCreatedFormSlug(updated.formSlug)
      setState("success")
    } catch (err) {
      setState("editing")
      setErrors({ submit: err instanceof ApiRequestError ? err.message : "Failed to update form" })
    }
  }
      ...
      <main className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6">
                ...
                <div className="mt-2 space-y-3">
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      disabled={state !== "editing" && state !== "submitting" || !newEmail.trim()}
                      variant="outline"
                    >
                      Add
                    </Button>
        ...
        <div className="flex justify-end gap-2 pb-8">
          ...
          >
            ...
            {state === "success" ? "Updated" : "Update Form"}
          </Button>
        </div>
        ...
        <Dialog open={state === "success"} onOpenChange={(open) => {
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Form Updated Successfully!</DialogTitle>
              <DialogDescription>
                Your form changes were saved. Share the link below if needed.
              </DialogDescription>
            </DialogHeader>
